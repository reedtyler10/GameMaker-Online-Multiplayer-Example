...
if (room == rm_game)
{
	...
	{
		if (pdata[_player].is_alive != 0) { continue; } //Player dead, don't draw him
		var alph = pdata[_player].can_die > 0 ? .4 : 1
		draw_set_color(c_white);
		draw_set_halign(fa_center);
		draw_text(pdata[_player].x, pdata[_player].y-48, pdata[_player].name);
		if (pdata[_player].is_player == 1)
		{
			draw_sprite_ext(spr_player, 0, pdata[_player].x, pdata[_player].y,1,1,pdata[_player].dir,c_white,alph);
		}
		else
		{
			draw_sprite_ext(spr_enemy, 0, pdata[_player].x, pdata[_player].y,1,1,pdata[_player].dir,c_white,alph);
		}
	}
